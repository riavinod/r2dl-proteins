...
class ksvd(object):
    ...
    def constructDictionary(self,inputMat,givenMat=None, epsilon):
        ...
        for iterNum in range(self.iteration):
            ...
            self.D=self.cleanDictionary(inputMat,self.D,coef, epsilon)
        return self.D
    ...
    def cleanDictionary(self,data,dictionary,coefMatrix, epsilon):
        T1=3
        T2=0.99
        ...
        for j in range(dictionary.shape[1]):
            if np.max(G[j,:])>T2 or np.sum(np.abs(coefMatrix[j,:])>epsilon)<=T1:
